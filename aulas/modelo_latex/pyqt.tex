...
\title{Interfaces gráficas desktop com PyQt5}
\date{\today}
...
\begin{document}
    ...
    \section{Conceitos básicos de PyQt5}
    
    \begin{frame}{Introdução ao PyQt5}
        
        \textbf{PyQt5} é um conjunto de bindings Python para o framework gráfico \textbf{Qt 5}.
    ...
    \begin{frame}[fragile]{Estrutura de uma aplicação PyQt5}
        
        Toda aplicação PyQt5 segue o mesmo padrão:
...
sys.exit(app.exec())
    ...
    \begin{frame}[fragile]{Sinais e Slots}
        ...
        PyQt5 usa o modelo de \textbf{sinais e slots} para eventos.
...
def ao_clicar():
    print("Botão clicado!")
    QMessageBox.information(botao, "Mensagem", "Botão clicado!")

botao.clicked.connect(ao_clicar) # Conecta sinal ao slot.
botao.show()
...
app.exec()
    ...
    \begin{frame}{Boas práticas em PyQt5}
    ...
    \begin{frame}{Aplicação PyQt5 — Qt Designer}
    ...
    \begin{frame}[fragile]{Aplicação PyQt5 — Estrutura mínima}
...
projeto/
    ...
    \begin{frame}[fragile]{Aplicação PyQt5 — Código mínimo}
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setWindowTitle("Aplicação PyQt5 mínima")
...
if __name__ == "__main__":
    ...
    \begin{frame}[fragile]{PyQt5 — Acessando widgets do .ui}
...
class MainWindow(QMainWindow):
    ...
    \begin{frame}[fragile]{PyQt5 — Conectando sinais}
...
class MainWindow(QMainWindow):
    ...
    \begin{frame}[fragile]{PyQt5 — Sinais e Slots com .ui}
...
def processar(self):
    ...
    \begin{frame}{PyQt5 — Diagrama UML da aplicação}
    ...
    \section{Widgets mais comuns}
    
    \begin{frame}[fragile]{QPushButton}
        ...
        \begin{columns}
            ...
            \begin{itemize}
                \item \texttt{text}
                \item \texttt{enabled}
                \item \texttt{checkable}
            \end{itemize}
            
            \column{0.48\textwidth}
            \textbf{Slots}
            \begin{itemize}
                \item \texttt{setText(str)}
                \item \texttt{setEnabled(bool)}
            \end{itemize}
            
            \textbf{Exemplo:}
            
            \includegraphics[width=0.3\linewidth]{pyqt_figs/pushbutton}
        ...
        \begin{minted}{python}
self.pushButton.clicked.connect(self.executar)
        \end{minted}
        
    \end{frame}
    
    \begin{frame}[fragile]{QComboBox}
        ...
        \begin{columns}
            ...
            \textbf{Signals}
            \begin{itemize}
                \item \texttt{currentIndexChanged(int)}
                \item \texttt{currentTextChanged(str)}
            \end{itemize}
            ...
            \textbf{Slots}
            \begin{itemize}
                \item \texttt{addItem(str)}
                \item \texttt{setCurrentIndex(int)}
            \end{itemize}
            
            \textbf{Exemplo:}
            
            \includegraphics[width=0.4\linewidth]{pyqt_figs/combobox}
        \end{columns}
...
texto = self.comboBox.currentText()
    ...
    \begin{frame}[fragile]{QListWidget}
        ...
        \begin{columns}
            ...
            \textbf{Signals}
            \begin{itemize}
                \item \texttt{itemClicked()}
                \item \texttt{currentItemChanged()}
            \end{itemize}
            
            \textbf{Properties}
            
            \begin{itemize}
                \item \texttt{currentRow}
                \item \texttt{count}
            \end{itemize}
            ...
            \includegraphics[width=0.6\linewidth]{pyqt_figs/listwidget}
        \end{columns}
...
item = self.listWidget.currentItem().text()
    ...
    \begin{frame}[fragile]{QLineEdit}
        ...
        \begin{columns}
            ...
            \textbf{Signals}
            \begin{itemize}
                \item \texttt{textChanged(str)}
                \item \texttt{returnPressed()}
                \item \texttt{editingFinished()}
            \end{itemize}
            \textbf{Properties}
            
            \begin{itemize}
                \item \texttt{text}
                \item \texttt{placeholderText}
            \end{itemize}
            ...
            \includegraphics[width=0.6\linewidth]{pyqt_figs/lineedit}
        \end{columns}
...
texto = self.lineEdit.text()
    ...
    \begin{frame}[fragile]{QPlainTextEdit}
        ...
        \begin{columns}
            ...
            \textbf{Slots}
            \begin{itemize}
                \item \texttt{setPlainText(str)}
                \item \texttt{appendPlainText(str)}
                \item \texttt{clear()}
            \end{itemize}
            \textbf{Exemplo:}
            
            \includegraphics[width=0.6\linewidth]{pyqt_figs/textedit}
        \end{columns}
...
texto = self.plainTextEdit.toPlainText()
    ...
    \begin{frame}[fragile]{QDateEdit}
        ...
        \begin{columns}
            ...
            \begin{itemize}
                \item \texttt{date}
                \item \texttt{minimumDate}
                \item \texttt{maximumDate}
            \end{itemize}
            
            \column[t]{0.48\textwidth}
            \textbf{Slots}
            \begin{itemize}
                \item \texttt{setDate(QDate)}
            \end{itemize}
            \textbf{Exemplo:}
            
            \includegraphics[width=0.3\linewidth]{pyqt_figs/dateedit}
        \end{columns}
...
data = self.dateEdit.date()
    ...
    \begin{frame}[fragile]{QDoubleSpinBox}
        
        Entrada numérica com incremento.
        ...
        \begin{columns}
            ...
            \begin{itemize}
                ...
                \item \texttt{minimum}
                \item \texttt{maximum}
                \item \texttt{singleStep}
            \end{itemize}
            
            \column{0.48\textwidth}
            \textbf{Slots}
            \begin{itemize}
                \item \texttt{setValue(float)}
                \item \texttt{setRange(min,max)}
            \end{itemize}
            \textbf{Exemplo:}
            
            \includegraphics[width=0.3\linewidth]{pyqt_figs/doublespinbox}
        \end{columns}
...
\section{Plots da MatPlotLib em PyQt}
...
\section{PyInstaller: Arquivos executáveis com Python}

% Slide 1
\begin{frame}[fragile]{Empacotando PyQt5 com Pixi}
...
# Verifica o caminho do arquivo ui no pyinstaller
base_path = getattr(sys, '_MEIPASS', ".")

class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        loadUi(os.path.join(base_path,"main_windo.ui"), self)
        self.setWindowTitle("Aplicação PyQt5 mínima")
...
\begin{frame}[fragile]{Executando PyInstaller via Pixi}
    ...
    \begin{minted}{bash}
pyinstaller --onefile --windowed --add-data="main_window.ui:." main.py
    \end{minted}
