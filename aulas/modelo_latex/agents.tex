...
\title{Agentes de Inteligência Artificial}
\date{\today}
...
\begin{frame}{Frameworks de agentes}
    
    \textbf{Os frameworks de agentes} permitem:
    \begin{itemize}
...
\begin{frame}{Por que usar um framework?}
    
    É possível conectar diretamente à API de um servidor de modelo de linguagem, como Ollama (local), OpenAI, Google ou Anthropic. Então por que usar um framework?
    \pause
    ...
    Frameworks de agentes fornecem:
    \begin{itemize}
...
\begin{frame}{Componentes de um Agente}
...
\begin{frame}[fragile]{Agente no strands}
	\begin{minted}{python}
...
model_id="gemma4"               # Specify which model to use
)
...
\begin{frame}{NVIDIA Nemo agents}
\url{https://docs.nvidia.com/nemo/agent-toolkit/latest/}

Caderno colab de exemplo:

\url{https://colab.research.google.com/github/NVIDIA/NeMo-Agent-Toolkit/blob/main/examples/notebooks/adding_tools_to_agents.ipynb}
\end{frame}
\end{document}
