...
\begin{frame}[fragile]{Rasterio: Leitura em Janela (Windowed Reading)}
...
\begin{frame}[fragile]{Rasterio: Leitura Automática por Blocos (Block Windows)}
...
\begin{frame}[fragile]{Rasterio: Leitura de Cloud Optimized Geotiffs}
...
# Create an AWS session with unsigned requests enabled!
...
with rasterio.Env(aws_session):
    with rasterio.open(cog_url) as dataset:
        ...
        top_overview = len(overviews)-1
    with rasterio.open(cog_url, OVERVIEW_LEVEL=top_overview) as dataset:
        print(f"Overview {top_overview} shape:")
        print(f"{dataset.width} x {dataset.height}")
        overview_data = dataset.read(1, out_shape=(512, 512))
        show(overview_data)
plt.show()
    \end{minted}
