Sequência de aulas  
- [Aula 1 - Ambiente de desenvolvimento](introducao.pdf)  <!-- Pronto -->
- [Aula 2 - Git - Controle de versões de arquivos](GIT.pdf)  <!-- Pronto -->
- [Aula 2.1 - Estruturas de dados em python](estruturas_dados_python.pdf) <!-- Pronto -->
- [Aula 3 - Python orientado a objetos](Python_oop.pdf) <!-- Pronto -->
- [Aula 4 - Interfaces gráficas desktop com PyQt6](pyqt.pdf) <!-- Pronto -->
- [Aula 6 PyQGIS - Python no QGIS](pyqgis.pdf)
- [Aula 7 PyQGIS - plugins de interface](pyqgis_plugins.pdf)
- [Aula 7 Geopandas com cadernos Jupyter com e Leafmap](geopandas_jupyter_leafmap.pdf) <!-- Iniciado -->
https://py.geocompx.org/01-spatial-data  
- [Aula 8 Interfaces web rápidas](interfaces_web_rapidas)<!-- Iniciado -->
- [Aula 9 Geoprocessamento](geoprocessamento.pdf)
- [Aula 10 Operações espaciais no QGIS](operacoes_espaciais_qgis.pdf) <!-- Pronto -->
- [Aula 11 PyQGIS plugin de geoprocessamento](pyqgis_processing_plugin.pdf) <!-- Iniciado -->
- Aula XX Serviços web de geoinformação
https://drive.google.com/file/d/1x9_QKMC1RiGESCXSCmmUEdtg-oOd9kV7/view?usp=drive_link
